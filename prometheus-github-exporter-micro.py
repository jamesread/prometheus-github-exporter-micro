...
def get_repos(repos, users):
    ...
    for user in users:
        ...
        while True:
            ...
            res = requests.get(f"https://api.github.com/users/{user}/repos?page={page}", headers = REQUEST_HEADERS)
...
while True:
    for repo in get_repos(REPOS, USERS):
        ...
        for wf in get_enabled_workflows(repo):
            ...
            if latest_run:
                logging.info("Latest run for workflow %s: %s", wf['name'], latest_run['status'])
